...
public class Sudoku {
    ...
    /// </summary>
    public byte[][] getData() {
        return m_sudoku.clone();

    }

    public void setData(byte[][] value){
        m_sudoku = value.clone();
    }
    ...
    public Pair<Long, Boolean> Generate(int spots, int numberOfTries)
    {
        // Number of set spots.
        int num = GetNumberSpots();                 //new = spots
        ...
        byte[][] originalData = getData();          //have data
        ...
        return Pair.create(tries, false);
    }
    ...
    private boolean Gen(int spots)
    {
        for (int i = 0; i < spots; i++)
        {
            ...
            do
            {
                xRand = getRandomizer().GetInt(9);
                yRand = getRandomizer().GetInt(9);

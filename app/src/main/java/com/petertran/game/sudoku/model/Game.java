...
public class Game {
    ...
    public void checkGame() {
        ...
        for (int y = 0; y < 9; y++) {
            for (int x = 0; x < 9; x++)
                check[y][x] = getGame()[y][x] == getSolution()[y][x];
        }
    }
    ...
    public boolean isSelectedNumberCandidate(int x, int y) {
        return getGame()[y][x] == 0 && isPossibleX(getGame(), y, selectedNumber)
                && isPossibleY(getGame(), x, selectedNumber) && isPossibleBlock(getGame(), x, y, selectedNumber);
    }
